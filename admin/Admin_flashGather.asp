...
If LCase(Request("Action")) <> "savenew" Then
	...
	Response.Write " <tr>"
	Response.Write "   <th>" & sModuleName & "FLASH采集管理</th>"
	Response.Write " </tr>"
	Response.Write " <tr>"
...
If Not CheckAdmin("flashCollect") Then
...
Class Cls_flashCollection
	...
	Private Sub Class_Initialize()
		...
		ChannelID = 5
		maxperpage = 30
		ScriptName = "Admin_flashGather.Asp"
		sModuleName = "动画"
		ChannelDir = "/flash/"
	End Sub
	...
	Public Sub ReloadflashItem(ItemID)
		If Not IsConnection Then DatabaseConnection
		Dim rsItem
		SQL = "SELECT * FROM [ECCMS_flashItem] WHERE ItemID=" & ItemID
		Set rsItem = MyConn.Execute(SQL)
		Myenchiasp.Value = rsItem.GetRows(1)
		Set rsItem = Nothing
	End Sub
	Public Sub ReloadflashConfig()
		If Not IsConnection Then DatabaseConnection
		SQL = "SELECT * FROM [ECCMS_flashConfig] "
		Set Rs = MyConn.Execute(SQL)
		Myenchiasp.Value = Rs.GetRows(1)
		Set Rs = Nothing
	End Sub
	Public Sub ReadflashConfig()
		On Error Resume Next
		
		Myenchiasp.Name = "flashConfig"
		If Myenchiasp.ObjIsEmpty() Then ReloadflashConfig
	...
	End Sub
	'--读取项目设置
	Public Sub ReadflashItem(ItemID)
		On Error Resume Next
		
		Myenchiasp.Name = "flashItem" & ItemID
		If Myenchiasp.ObjIsEmpty() Then ReloadflashItem (ItemID)
	...
	Private Sub showmain()
		...
		totalnumber = MyConn.Execute("SELECT COUNT(ItemID) FROM ECCMS_flashItem WHERE ChannelID=" & ChannelID)(0)
		...
		SQL = "SELECT ItemID,ItemName,SiteUrl,ChannelID,ClassID,SpecialID,StopItem,lastime,RemoteListUrl FROM [ECCMS_flashItem] WHERE ChannelID=" & ChannelID & " ORDER BY lastime DESC,ItemID DESC"
	...
	Private Sub SaveConfig()
		If Len(Request.Form("AllowPicExt")) = 0 Then
			...
			Exit Sub
		End If
		Myenchiasp.DelCahe ("flashConfig")
		Set Rs = CreateObject("ADODB.Recordset")
		SQL = "SELECT * FROM ECCMS_flashConfig WHERE id=1"
	...
	Private Sub CollectionItem(isEdit)
		...
		If isEdit Then
			Set Rs = MyConn.Execute("SELECT * FROM ECCMS_flashItem WHERE ChannelID=" & ChannelID & " And ItemID=" & ItemID)
			If Rs.BOF And Rs.EOF Then
				Set Rs = Nothing
				OutErrors ("错误的系统参数!")
				Exit Sub
	...
	Private Sub ItemStep2()
		...
		If Trim(Request("change")) = "yes" Then
			...
			Myenchiasp.DelCahe "flashItem" & ItemID
			...
			If ItemID = 0 Then
				SQL = "SELECT * FROM ECCMS_flashItem WHERE (ItemID is null)"
			Else
				SQL = "SELECT * FROM ECCMS_flashItem WHERE ItemID=" & ItemID
			End If
		...
		Set Rs = CreateObject("ADODB.Recordset")
		If ItemID = 0 Then
			Rs.Open "SELECT TOP 1 ItemID,FindListCode FROM ECCMS_flashItem WHERE ChannelID=" & ChannelID & " ORDER BY ItemID DESC", MyConn, 1, 1
		Else
			Rs.Open "SELECT ItemID,FindListCode FROM ECCMS_flashItem WHERE ChannelID=" & ChannelID & " And ItemID=" & ItemID & "", MyConn, 1, 1
		End If
	...
	Private Sub ItemStep3()
		...
		If Trim(Request("change")) = "yes" Then
			...
			SQL = "SELECT ItemID,FindListCode FROM ECCMS_flashItem WHERE ChannelID=" & ChannelID & " And ItemID=" & ItemID
			Set Rs = CreateObject("ADODB.Recordset")
			Rs.Open SQL, MyConn, 1, 3
			If Rs.BOF And Rs.EOF Then
				OutErrors ("错误的系统参数！")
		...
		End If
		'--获取项目设置
		SQL = "SELECT ItemID,Encoding,RemoteListUrl,FindListCode,FindInfoCode,IsNextPage,NamedDemourl FROM ECCMS_flashItem WHERE ChannelID=" & ChannelID & " And ItemID=" & ItemID
		Set Rs = MyConn.Execute(SQL)
		If Rs.BOF And Rs.EOF Then
			OutErrors ("错误的系统参数！")
			Set Rs = Nothing
		...
		With Response
			...
			.Write "<b>相关标签说明：</b><br>" & sModuleName & "标题 <font style='font-family:tahoma;color:red;'>{@flashTitle}</font>&nbsp;"
	...
	Private Sub ItemStep4()
		...
		Dim strflashTitle, flashContent, TextContent
		Dim TempHtmlCode, TempContent, strTempContent, PaginationUrl
		Dim datflashTime, strAuthor, strComeFrom
		...
		If Trim(Request("change")) = "yes" Then
			...
			SQL = "SELECT ItemID,FindInfoCode,IsNextPage,NamedDemourl FROM ECCMS_flashItem WHERE ChannelID=" & ChannelID & " And ItemID=" & ItemID
			Set Rs = CreateObject("ADODB.Recordset")
			Rs.Open SQL, MyConn, 1, 3
			If Rs.BOF And Rs.EOF Then
				OutErrors ("错误的系统参数！")
		...
		'--获取项目设置
		SQL = "SELECT ItemID,AutoClass,Encoding,RemoteListUrl,RemoveCode,FindListCode,FindInfoCode,IsNextPage,RetuneClass,NamedDemourl,strReplace FROM ECCMS_flashItem WHERE ChannelID=" & ChannelID & " And ItemID=" & ItemID
		Set Rs = MyConn.Execute(SQL)
		If Rs.BOF And Rs.EOF Then
			OutErrors ("错误的系统参数！")
			Set Rs = Nothing
		...
		With Response
			...
			If showcode > 0 Or LCase(Trim(Request("action"))) = "demo" Then
				...
				'--获取动画标题
				strflashTitle = Myenchiasp.CutFixed(HTTPHtmlCode, strFindInfoCode(0), strFindInfoCode(1))
				strflashTitle = Trim(Myenchiasp.CheckHTML(strflashTitle))
				If Len(strflashTitle) = 0 Then
					OutErrors ("获取标题代码出错！请确定你的代码输入正确。")
					Exit Sub
				End If
				
				'--获取动画内容
				flashContent = Myenchiasp.CutFixed(HTTPHtmlCode, strFindInfoCode(2), strFindInfoCode(3))
				If Len(flashContent) = 0 Then
					OutErrors ("获取动画内容代码出错！请确定你的代码输入正确。")
					Exit Sub
				End If
				...
				If strFindInfoCode(4) <> "" And strFindInfoCode(4) <> "0" Then
					startcode = Replace(strFindInfoCode(4), "{@flashTitle}", strflashTitle)
					lastcode = Replace(strFindInfoCode(5), "{@flashTitle}", strflashTitle)
				...
				If strFindInfoCode(6) <> "" And strFindInfoCode(6) <> "0" Then
					startcode = Replace(Replace(strFindInfoCode(6), "{@flashTitle}", strflashTitle), "{@ParentName}", strParent)
					lastcode = Replace(Replace(strFindInfoCode(7), "{@flashTitle}", strflashTitle), "{@ParentName}", strParent)
				...
				If strFindInfoCode(8) <> "" And strFindInfoCode(8) <> "0" Then
					startcode = Replace(Replace(Replace(strFindInfoCode(8), "{@flashTitle}", strflashTitle), "{@ParentName}", strParent), "{@ChildName}", strChild)
					lastcode = Replace(Replace(Replace(strFindInfoCode(9), "{@flashTitle}", strflashTitle), "{@ParentName}", strParent), "{@ChildName}", strChild)
				...
				If strFindInfoCode(10) <> "" And strFindInfoCode(10) <> "0" Then
					startcode = Replace(Replace(Replace(Replace(strFindInfoCode(10), "{@flashTitle}", strflashTitle), "{@ParentName}", strParent), "{@ChildName}", strChild), "{@flashAuthor}", strAuthor)
					lastcode = Replace(Replace(Replace(Replace(strFindInfoCode(11), "{@flashTitle}", strflashTitle), "{@ParentName}", strParent), "{@ChildName}", strChild), "{@flashAuthor}", strAuthor)
				...
				If strFindInfoCode(12) <> "" And strFindInfoCode(12) <> "0" Then
					startcode = Replace(Replace(Replace(Replace(Replace(strFindInfoCode(12), "{@flashTitle}", strflashTitle), "{@ParentName}", strParent), "{@ChildName}", strChild), "{@flashAuthor}", strAuthor), "{@flashComeFrom}", strComeFrom)
					lastcode = Replace(Replace(Replace(Replace(Replace(strFindInfoCode(13), "{@flashTitle}", strflashTitle), "{@ParentName}", strParent), "{@ChildName}", strChild), "{@flashAuthor}", strAuthor), "{@flashComeFrom}", strComeFrom)
					datflashTime = Myenchiasp.CutFixed(HTTPHtmlCode, startcode, lastcode)
					datflashTime = Myenchiasp.CheckHTML(datflashTime)
					datflashTime = Myenchiasp.CheckNostr(datflashTime)
					datflashTime = Myenchiasp.Formatime(Trim(datflashTime))
				Else
					datflashTime = Now
				End If
				...
				If CInt(IsNextPage) > 0 And strFindInfoCode(14) <> "" And strFindInfoCode(14) <> "0" And strFindInfoCode(15) <> "" And strFindInfoCode(15) <> "0" Then
					...
					If strFindInfoCode(16) <> "" And strFindInfoCode(16) <> "0" And strFindInfoCode(17) <> "" And strFindInfoCode(17) <> "0" Then
						If CInt(IsNextPage) = 1 Then
							'--从内容中读取分页获取列表
							strAddedCode = Myenchiasp.CutFixate(flashContent, strFindInfoCode(14), strFindInfoCode(15))
						Else
							'--从整个HTML代码中获取列表
						...
						If Len(strAddedCode) > 0 Then
							...
							If Len(strAddedlist) > 0 Then
								...
								For i = 0 To UBound(AddedlistArray)
									...
									End If
								Next
								flashContent = flashContent & strTempContent
								flashContent = Myenchiasp.CheckMatch(flashContent, strFindInfoCode(14), strFindInfoCode(15))
								flashContent = Replace(flashContent, "[page_break]", "<br /><span style=""color:red;font-size:12px;font-family:tahoma;font-weight:bold;"">此处是内容分页标签：[page_break]</span><br />")
							End If
						End If
					End If
				End If
				'----------------获取分页内容结束-------------------
				'--内容过滤
				TextContent = Myenchiasp.Html2Ubb(flashContent, RemoveCode)
				...
				strflashTitle = Myenchiasp.CheckNostr(strflashTitle)
	...
	Private Sub DataCollection()
		...
		fromPath = "tmpflashlist" & ItemID & ".dat"
	...
	Private Sub BeginCollection()
		...
		TmpFilePath = "tmpflashlist" & ItemID & ".dat"
	...
	Public Sub SaveflashData(URL)
		...
		Dim TempHtmlCode, TempContent, strTempContent, PaginationUrl
		Dim datflashTime, strAuthor, strComeFrom
		Dim flashBriefTopic, flashRelated
		Dim flashUploadFileList, flashImageUrl
		...
		flashContent = Myenchiasp.CutFixed(HTTPHtmlCode, strFindInfoCode(2), strFindInfoCode(3))
		If Len(flashContent) = 0 Then
			ReturnError ("获取动画内容代码出错！请确定你的代码输入正确。")
			Exit Sub
		End If
			...
			If strFindInfoCode(16) <> "" And strFindInfoCode(16) <> "0" And strFindInfoCode(17) <> "" And strFindInfoCode(17) <> "0" Then
				...
				If Len(strAddedCode) > 0 Then
					...
					If Len(strAddedlist) > 0 Then
						...
						flashContent = flashContent & strTempContent
						flashContent = Myenchiasp.CheckMatch(flashContent, strFindInfoCode(14), strFindInfoCode(15))
						'flashContent = Replace(flashContent, "[page_break]", "")
					End If
				End If
			End If
		End If
		...
		'--重新格式化动画标题
		strflashTitle = Myenchiasp.CheckNostr(strflashTitle)
		strflashTitle = Myenchiasp.FormatStr(strflashTitle)
		If CLng(AllHits) = 999 Then AllHits = Myenchiasp.GetRndHits
		'--动画关键字
		flashRelated = strflashTitle
		flashRelated = Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(flashRelated, "|", ""), "[", ""), "]", ""), "<", ""), ">", ""), "'", ""), """", ""), "$", "")
		flashRelated = Left(flashRelated, 4) & "|" & Right(flashRelated, 4)
		...
		Response.Write "<font color=red>" & strflashTitle & "</font> &nbsp;&nbsp;<br>"
		...
		If Myenchiasp.PictureEx Then
			...
			If CInt(IsDown) > 0 Then
				...
				flashUploadFileList = Myenchiasp.AllFileName
				FileNameArray = Split(flashUploadFileList, "|")
		...
		'--开始入库
		Set Rs = CreateObject("ADODB.Recordset")
		SQL = "SELECT * FROM ECCMS_flash WHERE title='" & strflashTitle & "'"
		...
		If IsUpdates Then
			If CInt(stopGather) <> 9 Then
				...
				Rs("title") = strflashTitle
				Rs("ColorMode") = 0
				Rs("FontMode") = 0
				Rs("content") = TextContent
				Rs("Related") = enchiasp.ChkFormStr(Left(flashRelated, 200))
				...
				Rs("MonthHits") = 0
				Rs("HitsTime") = Now()
				Rs("WriteTime") = datflashTime
				Rs("HtmlFileDate") = Trim(enchiasp.HtmlRndFileName)
				Rs("username") = Trim(AdminName)
				Rs("isBest") = 0
				Rs("BriefTopic") = flashBriefTopic
				Rs("ImageUrl") = Trim(flashImageUrl)
				Rs("UploadImage") = Trim(flashUploadFileList)
				Rs("UserGroup") = 0
				Rs("PointNum") = 0
				Rs("isUpdate") = 1
				Rs("isAccept") = 1
				Rs("ForbidEssay") = 0
				Rs("AlphaIndex") = enchiasp.ReadAlpha(strflashTitle)
				Rs.Update
			End If
			strMessage = "采集成功"
			strInfo = "恭喜您！采集成功"
	...
	Private Sub DeleteItem()
		If Trim(Request("ItemID")) <> "" Then
			MyConn.Execute ("DELETE FROM ECCMS_flashItem WHERE ItemID in (" & Request("ItemID") & ")")
	...
	Private Sub CopyNewItem()
		...
		If ItemID = 0 Then
			...
			Exit Sub
		End If
		Set rsCollect = MyConn.Execute("SELECT * FROM ECCMS_flashItem WHERE ChannelID=" & ChannelID & " And ItemID=" & ItemID)
