...
Private Sub PageTop()
	...
	Response.Write "<option value='admin_yemian.asp?ChannelID=" & ChannelID & "'>≡全部" & sModuleName & "列表≡</option>" & vbCrLf
	...
	Response.Write "	</tr>"
	Response.Write "	<tr>"
	Response.Write "	  <td colspan=2 class=TableRow2><b>操作选项：</b> <a href='admin_yemian.asp?ChannelID=" & ChannelID & "'>管理首页</a> | "
	Response.Write "	  <a href='admin_yemian.asp?ChannelID=" & ChannelID & "&action=add'>添加" & sModuleName & "</a> | "
	...
	Response.Write "	  <a href='admin_createyemian.asp?ChannelID=" & ChannelID & "'>生成HTML</a></td>"
	Response.Write "	</tr>"
...
Private Sub ArticleEdit(isEdit)
	...
	If isEdit Then
		...
		if rs.eof then
			FoundErr = True
			ErrMsg = ErrMsg + "<li>" & sModuleName & "该栏目，当前没有内容，请先添加内容！</li>"
			exit sub
			'Server.Transfer("showerr.asp")
			'Response.End
		end if
...
%>
    ...
    <input type=hidden name="BriefTopic" id="BriefTopic" type="Hidden"  value="0" >
           <input name="title" type=hidden type="text" id="title" size="60" value="运城向阳学校"> 
           ...
           <input name="ColorMode" type=hidden type="text" id="ColorMode" size="60" value="0">
			...
			<input name="Related" type=hidden type="text" id="Related" size="60" value="运城向阳学校"> 
             <input name="Author" type=hidden type="text" size="30" value="运城向阳学校">  
             <input name="ComeFrom" type=hidden type="text" size="30" value="运城向阳学校">
           ...
           <input type=hidden name=UploadFileList id=UploadFileList onchange="doChange(this,document.myform.ImageFileList)" value="<%If isEdit Then Response.Write Rs("UploadImage")%>">
			...
			<input type="Hidden" type='button' name='selectpic' value='从已上传图片中选择' onclick='SelectPhoto()' class=button>
...
Private Sub SaveArticle()
	...
	if not(rs.eof or rs.bof) then
		FoundErr = True
		ErrMsg = ErrMsg + "<li>此页面为单页面，不允许新添加内容，您已经添加了一个页面，请更改分类再继续！</li>"
	end if
	rs.close
	...
	If CInt(enchiasp.IsCreateHtml) <> 0 Then
		...
		Set Rs = Nothing
	End If
	Succeed("<li>恭喜您！添加新的" & sModuleName & "成功。</li><li><a href=admin_yemian.asp?action=view&ChannelID=" & ChannelID & "&ArticleID=" & ArticleID & ">点击此处查看该" & sModuleName & "</a></li><li><a href=?action=add&ChannelID=" & ChannelID & "&classid=" & Request.Form("ClassID") & "><font color=blue>点击此处继续添加" & sModuleName & "</font></a></li>")
End Sub
...
Private Sub ModifyArticle()
	...
	If CInt(enchiasp.IsCreateHtml) <> 0 Then
		...
		Call ScriptCreation(url,ArticleID)
	End If
	Succeed("<li>恭喜您！修改" & sModuleName & "成功。</li><li><a href=admin_yemian.asp?action=view&ChannelID=" & ChannelID & "&ArticleID=" & ArticleID & ">点击此处查看该" & sModuleName & "</a></li>")
End Sub
