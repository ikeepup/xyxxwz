...
'=====================================================================
If Not ChkAdmin("flashtupian") Then
...
%>
 <table cellspacing=1 align=center cellpadding=3 border=0 class=tableborder>
    ...
    <tr>
    <td class=tablerow2> 
    调用方法：在需要调用的地方加载如下标签：{$tupianhuan},如果要修改该FLASH的图片大小等参数请在通栏模版基本设置中修改。最多可设置10张图片，建议使用JPG图片
	</td>
    </table>
<br />
...
Sub Show()
	...
	dim pictxt(9)
	FoundErr=False
	Set rsInfo = enchiasp.Execute("select * From eccms_tupian order by id")
...
%>
<form method="POST" name="myform" onSubmit="Submit;" action="Admin_tupian.asp">
...
<th colspan="3">图片10</th>
 ...
 <tr class="tdbg"> 
   ...
   <tr class="tdbg"> 
	...
	<input name="Cancel" type="button" id="Cancel" value=" 取 消 " onClick="window.location.href='Admin_tupian.Asp'" style="cursor:hand;">
    </p>
  </div>
</form>
...
Sub DoSaveRecord()
	...
	next 
		...
		for i=1 to 10
			'if pic(i-1)<>"" then
				enchiasp.Execute("update eccms_tupian set pic='"& pic(i-1) &"',picurl='"& picurl(i-1) &"',pictext='"& pictxt(i-1) &"' where id="& i)
			'end if
		next
