...
function getMenuRow(s_Disabled, s_Event, s_Image, s_Html) {
	...
	s_MenuRow += ">"
	if (s_Image !=""){
		s_MenuRow += "&nbsp;<img border=0 src='images/"+config.StyleDir+"/"+s_Image+"' width=20 height=20 align=absmiddle "+s_Disabled+">&nbsp;";
	}else{
		s_MenuRow += "&nbsp;";
	}
...
function getFormatMenuRow(menu, html, image){
	var s_Disabled = "";
	if (!enchicms.document.queryCommandEnabled(menu)){
		s_Disabled = "disabled";
	}
...
function showContextMenu(event){
	...
	oPopupMenu.show(lefter, topper, width, height, enchicms.document.body);
	return false;
...
function showToolMenu(menu){
	...
	case "file":		// 文件视图菜单
		...
		sMenu += getMenuRow("", "ShowDialog('dialog/about.htm','400','220')", "about.gif", "关于恩池软件");
		sMenu += getMenuRow("", "window.open('http://www.enchi.com.cn')", "site.gif", "恩池软件");
		height = 228;
		break;
	case "table":		// 表格菜单
